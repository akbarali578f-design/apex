...
# QUEEN_I31_AM_TORCHLOAD_COMPAT_START
def _queen_torch_load_trusted_checkpoint(checkpoint_path, device=None):
    # Load trusted local QUEEN checkpoints under PyTorch >= 2.6.
    # PyTorch 2.6 changed torch.load(weights_only=True) by default.
    # AM needs the full local checkpoint object, so use weights_only=False.
    try:
        if device is None:
        ...
        if device is None:
            return _queen_torch_load_trusted_checkpoint(checkpoint_path, device=locals().get('device', None))
        return torch.load(checkpoint_path, map_location=device)
# QUEEN_I31_AM_TORCHLOAD_COMPAT_END
...
class AM(Blackbox):
    ...
    def from_modeldir(cls, model_dir, device=None, defense_level=0.99, rand_fhat=False, use_adaptive=True, output_type='probs', **kwargs):
        ...
        checkpoint = _queen_torch_load_trusted_checkpoint(checkpoint_path, device=locals().get('device', None))
        ...
        if not rand_fhat:
            ...
            model_def.load_state_dict(_queen_torch_load_trusted_checkpoint(model_def_path, device=locals().get('device', None)))
