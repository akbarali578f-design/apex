...
class MAD(Blackbox):
    def __init__(self, epsilon=None, optim='linesearch', model_adv_proxy=None, max_grad_layer=None, ydist='l1',
        ...
        # HGROUP PATCH: force MAD self.modelfamily from kwargs/local/proxy -- capture before Blackbox consumes kwargs
        _hgroup_mad_modelfamily = kwargs.pop("modelfamily", None)
        if _hgroup_mad_modelfamily is None:
            _hgroup_mad_modelfamily = locals().get("modelfamily", None)
        if _hgroup_mad_modelfamily is None:
            _hgroup_proxy_path = str(locals().get("model_adv_proxy", "")).lower()
            if "caltech256" in _hgroup_proxy_path or "caltech_256" in _hgroup_proxy_path or "caltech-256" in _hgroup_proxy_path:
                _hgroup_mad_modelfamily = "Caltech256"
            elif "cubs200" in _hgroup_proxy_path or "cub200" in _hgroup_proxy_path or "cub_200" in _hgroup_proxy_path or "cub-200" in _hgroup_proxy_path:
                _hgroup_mad_modelfamily = "CUBS200"
        super().__init__(*args, **kwargs)
        if _hgroup_mad_modelfamily is not None:
            self.modelfamily = _hgroup_mad_modelfamily
            print("=> HGROUP PATCH: MAD self.modelfamily={}".format(self.modelfamily))
        ...
        if model_adv_proxy is not None:
            ...
            # HGROUP PATCH2: force MAD proxy modelfamily immediately before zoo.get_net
            _hgroup_proxy_modelfamily = None
            if "_hgroup_mad_modelfamily" in locals():
                _hgroup_proxy_modelfamily = _hgroup_mad_modelfamily
            if _hgroup_proxy_modelfamily is None:
                _hgroup_proxy_modelfamily = getattr(self, "modelfamily", None)
            if _hgroup_proxy_modelfamily is None and "modelfamily" in locals():
                _hgroup_proxy_modelfamily = modelfamily
            if _hgroup_proxy_modelfamily is None:
                _hgroup_proxy_path = str(model_adv_proxy).lower()
                if "caltech256" in _hgroup_proxy_path or "caltech_256" in _hgroup_proxy_path or "caltech-256" in _hgroup_proxy_path:
                    _hgroup_proxy_modelfamily = "Caltech256"
                elif "cubs200" in _hgroup_proxy_path or "cub200" in _hgroup_proxy_path or "cub_200" in _hgroup_proxy_path or "cub-200" in _hgroup_proxy_path:
                    _hgroup_proxy_modelfamily = "CUBS200"
            if _hgroup_proxy_modelfamily is None:
                raise RuntimeError("HGROUP PATCH2 failed: cannot infer MAD proxy modelfamily from model_adv_proxy={}".format(model_adv_proxy))
            self.modelfamily = _hgroup_proxy_modelfamily
            print("=> HGROUP PATCH2: MAD proxy modelfamily={}".format(self.modelfamily))
